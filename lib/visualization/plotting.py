...
class VisualDataSource():
    def __init__(self, gt_path, pred_path):
        ...
        self.source=ColumnDataSource(data=dict(gtx=[], gty=[],
                                            px=[], py=[],
                                            diffx=[], diffy=[],
                                            disx=[], disy=[]))
        self.plot=0
    ...
    def visualize_paths(self, gt_path, pred_path, html_tile="", title="VO exercises", file_out="plot.html"):
        ...
        fig1.line("px", "py", source=self.source, color="green", legend_label="Pred")

        fig1.multi_line("disx", "disy", source=self.source, legend_label="Error", color="red", line_dash="dashed")
        fig1.legend.click_policy = "hide"

        fig2 = figure(title="Error", tools=tools, width_policy="max", toolbar_location="above",
                    x_axis_label="frame", y_axis_label="error")
        fig2.circle("diffx", "diffy", source=self.source, hover_fill_color="firebrick", legend_label="Error")
        fig2.line("diffx", "diffy", source=self.source, legend_label="Error")
    ...
    def plot_residual_results(self, qs_small, small_residual_init, small_residual_minimized,
                            qs, residual_init, residual_minimized):
        #output_file("plot.html", title="Bundle Adjustment")
        x = np.arange(2 * qs_small.shape[0])
        fig1, fig2, fig3 = self.make_residual_plot(x, small_residual_init, small_residual_minimized)

        x = np.arange(2 * qs.shape[0])
        fig4, fig5, fig6 = self.make_residual_plot(x, residual_init, residual_minimized)
    ...
    def visualize_update(self, gt_path, pred_path):
        gt_path = np.array(gt_path)
        pred_path = np.array(pred_path)
